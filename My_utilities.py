...
def load_validation_data(frequency, velocity_model, dtype=tf.float32, use_PML=False):
    ...
    elif velocity_model == 'smooth_marmousi':
        mat_data = scipy.io.loadmat(f'processed_data/FD_results_{frequency}Hz_val_velocity_marmousi.mat')
        a_x, b_x = 0., 2.5  # Bounds for x
        a_z, b_z = 0., 2.5  # Bounds for z
    else:
...
def precreate_xz_reg(xz, s_xz, omega, v0,beta=1000, num_reg_points=500,max_bound=2.):
    """
    生成源点附近的正则化点和正则化因子，支持两种正则化因子计算方式：
    1. gaussian: 高斯衰减（推荐，平滑性好，符合波场物理规律）
    2. inverse_distance: 距离倒数（近源点权重极高，适合强约束场景）
    """
    reg_type = 'gaussian'
    # 计算每个点到源点的平方距离和实际距离（避免平方距离的数值偏差）
    distance_squared = tf.reduce_sum(tf.square(xz - s_xz), axis=-1)
    distance = tf.sqrt(distance_squared)  # 实际欧氏距离

    # 选取距离源点最近的num_reg_points个点的索引
    _, indices = tf.nn.top_k(-distance_squared, k=num_reg_points)  # 负号实现"最小距离"排序

    # 提取源点附近的正则化点坐标
    xz_reg = tf.gather(xz, indices)
    # 提取正则化点对应的距离（用于计算因子）
    reg_distance = tf.gather(distance, indices)
    reg_distance_squared = tf.gather(distance_squared, indices)

    # 基于波长计算正则化的空间尺度（核心：与波场物理尺度绑定）
    lambda_ = 2 * np.pi * v0 / omega  # 波长 λ = 2πv/ω
    sigma = lambda_ / (2 * max_bound)  # 高斯宽度/距离阈值，与波长和max_bound关联

    # 计算正则化因子（两种方式可选）
    if reg_type == 'gaussian':
        # 方式1：高斯衰减（推荐）- 平滑衰减，符合波场能量传播规律
        factor_d = beta ** 2 * omega * tf.exp(-reg_distance_squared / (2 * sigma ** 2))
    elif reg_type == 'inverse_distance':
        # 方式2：距离倒数 - 近源点权重极高，强约束（需避免除零）
        safe_distance = tf.maximum(reg_distance, 1e-8)  # 防止距离为0导致的数值爆炸
        factor_d = beta ** 2 * omega / safe_distance
    else:
        raise ValueError(f"reg_type must be 'gaussian' or 'inverse_distance', got {reg_type}")

    # 扩展维度为[num_reg_points, 1]，方便后续损失函数加权计算
    factor_d = tf.expand_dims(factor_d, axis=-1)
